                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-xs {{ $usersChange >= 0 ? 'text-green-500' : 'text-red-500' }} mt-1">
                                    {{ $usersChange >= 0 ? '+' : '' }}{{ $usersChange }}% desde el mes pasado
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $totalTransactions }}</p>
                                <p class="text-xs {{ $transactionsChange >= 0 ? 'text-green-500' : 'text-red-500' }} mt-1">
                                    {{ $transactionsChange >= 0 ? '+' : '' }}{{ $transactionsChange }}% desde el mes pasado
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-xs {{ $balanceChange >= 0 ? 'text-green-500' : 'text-red-500' }} mt-1">
                                    {{ $balanceChange >= 0 ? '+' : '' }}{{ $balanceChange }}% desde el mes pasado
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $totalBanks ?? '0' }}</p>
                                <p class="text-xs {{ $banksChange >= 0 ? 'text-green-500' : 'text-red-500' }} mt-1">
                                    {{ $banksChange >= 0 ? '+' : '' }}{{ $banksChange }}% desde el mes pasado
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{{ $movement->user->email ?? 'Usuario desconocido' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                        {{ $movement->motivo ?? 'Movimiento' }}
                                        @if($movement->tipo_movimiento === 'transferencia' && $movement->bancoEmisor && $movement->bancoReceptor)
                                            <div class="mt-1 text-xs text-gray-400">
                                                {{ $movement->bancoEmisor->nombre ?? 'Banco origen' }} → 
                                                {{ $movement->bancoReceptor->nombre ?? 'Banco destino' }}
                                            </div>
                                        @elseif($movement->tipo_movimiento === 'ingreso' && $movement->bancoReceptor)
                                            <div class="mt-1 text-xs text-gray-400">
                                                {{ $movement->bancoReceptor->nombre ?? 'Cuenta destino' }}
                                            </div>
                                        @elseif($movement->bancoEmisor)
                                            <div class="mt-1 text-xs text-gray-400">
                                                {{ $movement->bancoEmisor->nombre ?? 'Cuenta origen' }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            ...
                                            @elseif($movement->tipo_movimiento === 'egreso')
                                                bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100
                                            @elseif($movement->tipo_movimiento === 'transferencia')
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $movement->created_at->diffForHumans() }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
    ...
    <div id="modal-mov" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative mx-auto mt-16 w-full max-w-2xl p-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                ...
                <form method="POST" action="{{ route('movimientos.store') }}" class="p-6 space-y-4">
                    @csrf
                ...
                <form id="form-reporte" method="GET" class="p-6 space-y-4">
                    ...
                    <div class="flex items-center justify-between gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex gap-3">
                            <a id="btn-ver-reporte" href="{{ route('auditorias.reportes') }}" class="px-4 py-2 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Ver en pantalla</a>
                            <a id="btn-descargar-pdf" href="{{ route('auditorias.generar-reporte-pdf') }}" target="_blank" class="px-4 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-700">Descargar PDF</a>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        if (transactionsChartEl) {
            ...
            const transactionsChart = new Chart(transactionsCtx, {
                type: 'line',
                data: {
                    labels: {!! json_encode($transactionsData['labels'] ?? []) !!},
                    datasets: [{
                        label: 'Transacciones',
                        data: {!! json_encode($transactionsData['data'] ?? []) !!},
        ...
        if (categoriesChartEl) {
            ...
            const categoryData = {!! json_encode($categoryData ?? [], JSON_PRETTY_PRINT) !!};
        ...
        if (banksChartEl) {
            ...
            const banksData = {!! json_encode($banksData ?? [], JSON_PRETTY_PRINT) !!};
